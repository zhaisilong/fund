...
class Trace:
    def __init__(self, path: Union[str, Path], fund: Fund, buy_fee: float = 0.0015):
        if isinstance(path, str):
            path = Path(path)
        elif isinstance(path, Path):
            path = path
    ...
    def _build(self) -> Tuple[Pool]:
        ...
        for i, row in self.trace_db.df.iterrows():
            ...
            if date > self.fund.latest_day:
                logger.info(f'最新一天尚未更新，请于明日数据更新后再试，此条记录将不纳入追踪。')
    ...
    def _report(self, parent: Path):
        ...
        if len(pool_pro) == 0:  # pool 里无记录
            logger.info(f'没有最新数据,不做记录{self.name}-{self.code}')
            return 0
